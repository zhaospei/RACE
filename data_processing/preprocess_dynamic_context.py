        ...
        for x, i in enumerate(data):
            ...
            for line in ls:
                ...
                if line.startswith('ppp ') or line.startswith('mmm '):
                    if len(del_list) != 0 or len(add_list) != 0 or len(same_list) != 0:
                        if len(add_list) == len(del_list):
                            count_remove += 1
                            for _del in del_list:
                                diff_tokens.append(_del)
                            for _add in add_list:
                                diff_tokens.append(_add)
                        else:
                            for _same in same_list:
                                diff_tokens.append(_same)
                    del_list = []
                    add_list = []
                    same_list = []
                    tokens, _, _ = subtokenize_code(line[3:], lang[cur_lang + 1])
                    ...
                    add_list = []
                    same_list = []
                    tokens, _, _ = subtokenize_code(line[3:], lang[cur_lang])
                    same_list.append('<KEEP>')
                    for token in tokens:
                        same_list.append(token)
                    same_list.append('<KEEP_END>')
                else:
                    st = line[0]
                    if nxt_file:
                        cur_lang += 1
                    ...
                    if st != '+' and st != '-': # the code not changed 
                        try:
                            tokens, _, _ = subtokenize_code(line, lang[cur_lang])
                            same_list.append('<KEEP>')
                            for token in tokens:
                                same_list.append(token)
                            same_list.append('<KEEP_END>')
                        ...
                        line = line[1:].strip()
                        tokens, _, _ = subtokenize_code(line, lang[cur_lang])
                        del_list.append('<DELETE>')
                        same_list.append('<DELETE>')
                        for token in tokens:
                            del_list.append(token)
                            same_list.append(token)
                        del_list.append('<DELETE_END>')
                        same_list.append('<DELETE_END>')
                    elif st == '+': # the code added
                        line = line[1:].strip()
                        tokens, _, _ = subtokenize_code(line, lang[cur_lang])
                        add_list.append('<INSERT>')
                        same_list.append('<INSERT>')
                        for token in tokens:
                            add_list.append(token)
                            same_list.append(token)
                        add_list.append('<INSERT_END>')
                        same_list.append('<INSERT_END>')
            if len(del_list) != 0 or len(add_list) != 0 or len(same_list) != 0:
                if len(add_list) == len(del_list):
                    count_remove += 1
                    for _del in del_list:
